...
def critical_calculation_inner(output_data, start_time_min):
    ...
    for k, v in output_data.items():
        ...
        for item in v:
            ...
            print("\t start time %.2fus ::: wait time %.2fus ::: hold time %.2fus"
            % (item.start_time - start_time_min, item.wait_time - start_time_min, item.lock_time - start_time_min))
            print("\t mtx %d ::: start block %d ::: wait block %d ::: hold block %d" % (item.mtx, start, wait, hold))
